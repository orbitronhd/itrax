...
export function HeroBanner({ imageUrl, scrollY, vh }: HeroBannerProps) {
  ...
  // 0 - 0.3vh: Text 1 fades out
  const text1Progress = Math.max(0, Math.min(1, scrollY / (0.3 * vh)));
  const text1Opacity = 1 - text1Progress;
  const text1Blur = text1Progress * 12;
  const text1TranslateY = text1Progress * 20;

  // 0.2vh - 0.6vh: Text 2 fades in
  const text2Progress = Math.max(0, Math.min(1, (scrollY - 0.2 * vh) / (0.4 * vh)));
